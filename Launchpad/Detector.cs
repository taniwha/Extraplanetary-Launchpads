            ...
            if (IsDetecting && this.vessel != null && this.vessel.gameObject.active && Altitude <= this.DetectingHeight)
            {
                ...
                if (TimerEcho >= TimerThreshold)
                {
                    if (DepositUnder != null) {
						if (DepositUnder.Quantity >= 1.0f)
	                    {
							if (DepositUnder is OreDeposit) {
								print ("Detected Ore Deposit");
								controller.DrawMap(2);
							} else {
	                        	controller.DrawMap(1);
							}
	                        ...
	                        if (vessel == FlightGlobals.ActiveVessel && controller.ScanningSound)
	                            PingDeposit.Play();
						}else if (float.IsNaN(DepositUnder.Quantity)) {
							DepositUnder.Quantity = 500000;
							print ("----------\n"+DepositUnder.Quantity.ToString());
						}
                    }
                    else
                    {
                        controller.DrawMap(0);
                        if (vessel == FlightGlobals.ActiveVessel && controller.ScanningSound)
                            PingEmpty.Play();
                    }
                    TimerEcho = 0;
